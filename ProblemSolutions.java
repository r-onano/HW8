...
class ProblemSolutions {
    ...
    public int numGroups(int[][] adjMatrix) {
        ...
        // Boolean to keep track of nodes that have been visited.
        boolean[] visited = new boolean[numNodes];

        // Initialize count for ghroups visited.
        int groups = 0;

        // Iterate through all nodes in the graph
        for (i = 0; i < numNodes; i++) {
            // If node not visited, then part of new group.
            if (!visited[i]) {
                // depth first seach to visit all nodes connectred to current
                depthFirst(i, graph, adjMatrix, visited);

                // count group after finish with visit.
                groups++;
            }
        }
    ...
    private void depthFirst(int node, Map<Integer, List<Integer>> graph, int[][] adjMatrix, boolean[] visited) {
        // Mark current node as visited to avoid double visit.
        visited[node] = true;
        ...
        if (graph.containsKey(node)) {
            ...
            for (int neighbor : graph.get(node)) {
                ...
                if (!visited[neighbor]) {
                    depthFirst(neighbor, graph, adjMatrix, visited);
                }
            }
        }
    }
